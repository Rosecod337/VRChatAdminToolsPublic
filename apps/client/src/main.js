...
function formatUpdaterError(error) {
  ...
  if (statusCode === 404 || raw.includes("404")) {
    return "GitHub не нашёл файл обновления. Повторите проверку позже.";
  }
  if (/latest\.yml|latest-mac\.yml|latest-linux\.yml/iu.test(raw)) {
    return "В выпуске GitHub нет файла обновления. Повторите проверку позже.";
  }
  if (/net::|ENOTFOUND|ECONNRESET|ETIMEDOUT|timeout|network/iu.test(raw)) {
    return "Не удалось подключиться к GitHub для проверки обновлений.";
  }
